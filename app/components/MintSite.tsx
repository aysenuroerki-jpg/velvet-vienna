...
const copy = {
  de: {
    ...
    studioKicker: "Velvet Vienna · Wien",
  ...
  return (
    <div className="site-shell" dir={rtl ? "rtl" : "ltr"}>
      ...
      <header className={page === "home" ? "site-header site-header--overlay" : "site-header"}>
        <Link className="brand" href="/" aria-label="Velvet Vienna home"><BrandLockup /></Link>
      ...
      <footer className="site-footer">
        ...
        <div><a href={`mailto:${contactEmail}`}>{contactEmail}</a><a href={whatsappUrl} target="_blank" rel="noreferrer" aria-label={`WhatsApp ${whatsappLabel}`}>WhatsApp · {whatsappLabel} ↗</a><a href="https://www.instagram.com/mintevents.vienna/" target="_blank" rel="noreferrer">Instagram ↗</a></div>
        <p className="copyright">© 2026 Velvet Vienna · Event &amp; Organization · {localeExtras[lang].city}</p>
      </footer>
...
function PortfolioPage({ t, filter, setFilter, items, open, lang }: { t: typeof copy.de; filter: string; setFilter: (value: string) => void; items: GalleryItem[]; open: (item: GalleryItem) => void; lang: Lang }) {
  ...
  return <>
    <section className="page-hero"><div className="page-hero__copy"><p className="kicker">{t.projectsKicker}</p><h1>{t.projectsTitle}</h1><p>{t.projectsText}</p><span className="page-count">{allProjects.length} · Velvet Vienna</span></div><div className="page-hero__image"><Picture item={heroItem} priority /></div></section>
...
function DetailsCatalog({ t, filter, setFilter, open, labels, lang }: { t: typeof copy.de; filter: string; setFilter: (value: string) => void; open: (item: GalleryItem) => void; labels: (typeof categoryLabels)[Lang]; lang: Lang }) {
  ...
  return <>
    <section className="page-hero"><div className="page-hero__copy"><p className="kicker">{t.detailsKicker}</p><h1>{t.detailsTitle}</h1><p>{t.detailsText}</p><span className="page-count">{count} · Velvet Vienna</span></div><div className="page-hero__image"><Picture item={ceremonyDetails[3]} priority /></div></section>
...
function Studio({ t, lang }: { t: typeof copy.de; lang: Lang }) {
  return <>
    ...
    <section className="studio-quote"><p>“</p><h2>{t.studioText}</h2><span>Velvet Vienna · Event &amp; Organization · {localeExtras[lang].city}</span></section>
    <Cta t={t} />
  </>;
}
